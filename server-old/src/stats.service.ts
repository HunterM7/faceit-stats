...
import { DuoMatchesResponse, LastMatchResponse, MatchHistoryItem, MatchResult, PlayerSnapshotResponse, StatsResponse } from "./types";
...
export class StatsService {
  ...
  async getStatsByNickname(rawNickname: string): Promise<StatsResponse> {
    ...
    if (!playerId) {
      throw new Error("FACEIT player not found by nickname");
    }
  ...
  async getLastMatchByPlayerId(playerId: string): Promise<LastMatchResponse> {
    ...
    if (!normalizedPlayerId) {
      throw new Error("playerId is empty");
    }
  ...
  async getDuoMatchesInCsgo(rawNickname: string, rawTeammateNickname: string): Promise<DuoMatchesResponse> {
    ...
    if (!nickname || !teammateNickname) {
      throw new Error("nickname and teammateNickname are required");
    }
